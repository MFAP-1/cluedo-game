...
// Final arrays
  ...
  let roomsArray = [
      diningRoom, conservatory, kitchen, study, library, billiardRoom, lounge, 
...
// Generates random item from an array to define the mystery
function pickMystery(){
    let randomSuspect = suspectsArray[Math.floor(Math.random() * suspectsArray.length)]; // for random suspect
    let randomRoom = roomsArray[Math.floor(Math.random() * roomsArray.length)]; // for random room
    let randomWeapon = weaponsArray[Math.floor(Math.random() * weaponsArray.length)]; // for random weapom
    
    return {randomSuspect, randomWeapon, randomRoom}; // returning an object
}

let mystery = pickMystery();


// Show arrays in the screen to help the users
document.body.innerHTML += `<h3> List that helps you choose </h3>`;
document.body.innerHTML += `<h4> Suspects: </h4>`;
showArray(suspectsArray);
document.body.innerHTML += `<h4> Rooms: </h4>`;
showArray(roomsArray);
document.body.innerHTML += `<h4> Weapons: </h4>`;
...
// Function to check the answered with the random generated 
...
function guess(){
    ...
    if (guessedSuspect === mystery.randomSuspect.name 
        && guessedRoom === mystery.randomRoom.name 
        && guessedWeapon === mystery.randomWeapon.name) {
        alert("You guessed right! You win!!");
    } else {
        alert("You guessed wrong =(");
        alert(mystery.randomSuspect.name + " killed mr Mfap in the " + mystery.randomRoom.name +" with " + mystery.randomWeapon.name);
    }
